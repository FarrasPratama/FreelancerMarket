...
<div class="container">
  <form data-parsley-validate class="col-sm-12" action="<?php echo base_url('uploadbukti/simpan_bukti') ?>" method="post" enctype="multipart/form-data">
    ...
    <br>
    <?php echo $transaksi[0]->transaksi_id ?> -->
      ...
      <tbody>
        <?php if (!empty($transaksi_sewa)) :
          foreach ($transaksi_sewa as $key => $p) {
            ...
            if ($p->status == '0') {
              $aksi = '<a href="' . base_url('data_transaksi_sewa/setuju/' . $p->transaksi_id . '/' . $p->produk_id) . '" class="btn btn-danger btn-xs">Upload Bukti Pembayaran</a>';
            } elseif ($p->status == '2') {
              $aksi = '-';
            } else {
